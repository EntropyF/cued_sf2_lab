...
from DWT import nlevdwt, nlevidwt, quantdwt, get_step_ratios
import numpy as np
...
from cued_sf2_lab.jpeg import (
    jpegenc, jpegdec, quant1, quant2, huffenc, huffdflt, huffdes, huffgen, diagscan, runampl, HuffmanTable, dwtgroup)
from typing import Tuple, NamedTuple, Optional
from cued_sf2_lab.bitword import bitword
from scan import row_wise_scan, column_wise_scan, zigzag_scan
...
def jpegenc_2dct(X: np.ndarray, qstep: float, N: int = 4, M: int = 16,
        opthuff: bool = False, dcbits: int = 16, log: bool = True
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Encodes the image in X to generate a variable length bit stream.
    ...
    Y[:64,:64] = colxfm(colxfm(Y[:64,:64], C4).T, C4).T
...
def jpegdec_2dct(vlc: np.ndarray, qstep: float, N: int = 4, M: int = 16,
        hufftab: Optional[HuffmanTable] = None,
        dcbits: int = 16, W: int = 256, H: int = 256, log: bool = True
        ) -> np.ndarray:
    '''
    Decodes a (simplified) JPEG bit stream to an image
    ...
    if log:
        print('Inverse {} x {} DCT\n'.format(N, N))
    C4 = dct_ii(N)
    Zi[:64,:64] = colxfm(colxfm(Zi[:64,:64].T, C4.T).T, C4.T)
    Z = colxfm(colxfm(Zi.T, C4.T).T, C4.T)
...
def jpegenc_dwt_const_step(X: np.ndarray, qstep: float, n: int = 3, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Encodes the image in X to generate a variable length bit stream.
...
def jpegdec_dwt_const_step(vlc: np.ndarray, qstep: float, n: int = 3, N: int = 8, M: int = 8,
        hufftab: Optional[HuffmanTable] = None,
        dcbits: int = 8, W: int = 256, H: int = 256, log: bool = True
        ) -> np.ndarray:
    '''
...
# step = 13
# vlc, dhufftab = jpegenc_dwt_const_step(Xl, step, opthuff=True)
# Z = jpegdec_dwt_const_step(vlc, step, hufftab=dhufftab)
# fig, ax = plt.subplots()
# plot_image(Z, ax=ax)
# plt.show()

def jpegenc_dwt_equal_mse(X: np.ndarray, qstep: float, n: int = 3, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Encodes the image in X to generate a variable length bit stream.
...
def jpegdec_dwt_equal_mse(vlc: np.ndarray, qstep: float, n: int = 3, N: int = 8, M: int = 8,
        hufftab: Optional[HuffmanTable] = None,
        dcbits: int = 8, W: int = 256, H: int = 256, log: bool = True
        ) -> np.ndarray:
    '''
...
def jpegenc1(X: np.ndarray, qstep: float, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Encodes the image in X to generate a variable length bit stream.
...
def jpegdec1(vlc: np.ndarray, qstep: float, N: int = 8, M: int = 8,
        hufftab: Optional[HuffmanTable] = None,
        dcbits: int = 8, W: int = 256, H: int = 256, log: bool = True
        ) -> np.ndarray:
    '''
...
def jpegenc2(X: np.ndarray, qstep: float, N: int = 8, M: int = 8,
        opthuff: bool = False, dcbits: int = 8, log: bool = True
        ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    '''
    Encodes the image in X to generate a variable length bit stream.
...
def jpegdec2(vlc: np.ndarray, qstep: float, N: int = 8, M: int = 8,
        hufftab: Optional[HuffmanTable] = None,
        dcbits: int = 8, W: int = 256, H: int = 256, log: bool = True
        ) -> np.ndarray:
    '''
